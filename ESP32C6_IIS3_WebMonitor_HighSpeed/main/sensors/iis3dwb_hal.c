...
esp_err_t iis3dwb_hal_read_data(stmdev_ctx_t *dev_ctx, iis3dwb_hal_data_t *data)
{
    ...
    return ret;
#else
    return iis3dwb_hal_read_polling_data(dev_ctx, data, 1); // Polling mode: read 1 sample per call (fast, no watchdog issues)
#endif
}
...
static esp_err_t iis3dwb_hal_read_polling_data(stmdev_ctx_t *ctx, iis3dwb_hal_data_t *data, uint8_t sample)
{
    ...
    // Store all samples (not just average) for WebSocket
    uint16_t samples_stored = 0;
    uint32_t timeout_count = 0;
    const uint32_t MAX_TIMEOUT = 100000; // Prevent infinite loop (100k iterations)
    ...
    for(int i = 0; i < sample; i++){
        ...
        do{
            ...
            if (!reg) {
                taskYIELD(); // Yield CPU without delay - allows IDLE task to run
                if (++timeout_count > MAX_TIMEOUT) {
                    ESP_LOGW(TAG, "Accel data timeout");
        ...
        // Store individual sample in extended data array
        if (samples_stored < IIS3DWB_HAL_MAX_SAMPLES && data->samples) {
            data->samples[samples_stored].x_raw = data_raw_acceleration[0];
            data->samples[samples_stored].y_raw = data_raw_acceleration[1];
            data->samples[samples_stored].z_raw = data_raw_acceleration[2];
            samples_stored++;
        }
        ...
        do{
            ...
            if (!reg) {
                taskYIELD(); // Yield CPU without delay
                if (++timeout_count > MAX_TIMEOUT) {
                    ESP_LOGW(TAG, "Temp data timeout");
                    return ESP_ERR_TIMEOUT;
                }
            }
        }while(!reg);
    ...
    data->sample_count = samples_stored;

    // Convert acceleration data to mg (average for compatibility)
