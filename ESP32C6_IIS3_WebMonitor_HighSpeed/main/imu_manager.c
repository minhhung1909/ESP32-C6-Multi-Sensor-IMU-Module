...
#define IIS3DWB_MAX_SAMPLES_BATCH 128

static SemaphoreHandle_t sensor_mutex = NULL;
static uint16_t fifo_watermark = 192;
...
{
    uint8_t status[2] = {0};
    esp_err_t ret = iis3dwb_read_reg(&accel_sensor, IIS3DWB_FIFO_STATUS1, status, sizeof(status));
    if (ret != ESP_OK) {
        return ret;
    }

    uint16_t diff_fifo = status[0] | ((status[1] & 0x07) << 8);
    if (level) {
        *level = diff_fifo;
    }
    if (overflowed) {
        *overflowed = (status[1] & 0x40U) != 0;
    }
    return ESP_OK;
}
...
esp_err_t imu_manager_init(void)
{
    ...
    ret = iis3dwb_init_spi(&accel_sensor, IIS3DWB_SPI_HOST, IIS3DWB_SPI_CS);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "IIS3DWB SPI init failed: %s", esp_err_to_name(ret));
        return ret;
    }

    ret = iis3dwb_device_init(&accel_sensor);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "IIS3DWB device init failed: %s", esp_err_to_name(ret));
        return ret;
    }
    ...
    ret = iis3dwb_fifo_config(&accel_sensor, fifo_watermark, IIS3DWB_FIFO_STREAM_MODE);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "IIS3DWB FIFO configuration failed: %s", esp_err_to_name(ret));
        return ret;
    }
...
esp_err_t imu_manager_read_accelerometer(imu_data_t *data)
{
    ...
    if (overflow) {
        ESP_LOGW(TAG, "IIS3DWB FIFO overflow detected");
    }
    ...
    if (samples_to_read == 0) {
        ...
        float ay = 0.0f;
        float az = 0.0f;
        ret = iis3dwb_read_accel(&accel_sensor, &ax, &ay, &az);
        if (ret == ESP_OK) {
    ...
    uint8_t fifo_raw[samples_to_read * 7];
    float ax_buf[samples_to_read];
    float ay_buf[samples_to_read];
    float az_buf[samples_to_read];

    ret = iis3dwb_fifo_read_burst(&accel_sensor, fifo_raw, samples_to_read);
    if (ret != ESP_OK) {
        data->accelerometer.valid = false;
        return ret;
    }

    iis3dwb_convert_raw_to_g(fifo_raw, samples_to_read, ax_buf, ay_buf, az_buf);

    const uint16_t last_index = samples_to_read - 1;
    ...
    if (last_batch_timestamp_us != 0 && now_us > last_batch_timestamp_us) {
        ...
        samples_per_second = (samples_to_read * 1e6f) / elapsed_us;
    }
    last_batch_timestamp_us = now_us;

    data->stats.fifo_level = fifo_level;
    data->stats.samples_read = samples_to_read;
    data->stats.odr_hz = configured_odr_hz;
    data->stats.batch_interval_us = (samples_to_read * 1e6f) / configured_odr_hz;
    ...
    memcpy(recent_ax, ax_buf, samples_to_read * sizeof(float));
    memcpy(recent_ay, ay_buf, samples_to_read * sizeof(float));
    memcpy(recent_az, az_buf, samples_to_read * sizeof(float));
    recent_samples = samples_to_read;
